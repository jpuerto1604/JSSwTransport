...
class JobShopEnv:
    ...
    def update_resource_states(self):
        ...
        This method checks the status of each machine and AGV. If a machine or AGV is currently in use 
        (status is 1) and the current time has reached or passed the time it is supposed to be available, 
        the method updates its status to free (status is 0) and sets its available time to infinity 
        (indicating it is currently unused). It also logs the availability of the machine or AGV.
        
        Attributes:
            machine_status (list of int): List indicating the status of each machine (1 for in use, 0 for free).
            machine_available_at (list of float): List indicating the time each machine will be available.
            agv_status (list of int): List indicating the status of each AGV (1 for in use, 0 for free).
            agv_available_at (list of float): List indicating the time each AGV will be available.
